...
if __name__ == '__main__':
    ...
    def init_model(model):
        ...
        if model == 'Transformer':
            ...
            # 使用自定义的 ResNet3D 模型
            net_model = ResNet3D(num_classes=2, input_channels=1).to(device)
            print("Using custom ResNet3D model")
        return net_model


    # 训练函数中的模型调用不变，确保在 init_model 中正确调用了 ResNet3D
    def train_model(train_dataloader, val_dataloader, test_dataloader, fold, weights):
        ...
        # 初始化模型、优化器以及损失函数
        net_model = init_model(opt.model)  # 调用 init_model() 获取 ResNet3D 模型
        ...
        def train_step(engine, batch):
            ...
            # 前向传播，模型返回的输出
            output_logits = net_model(MRI)  # 注意，这里是 ResNet3D 模型的前向传播
